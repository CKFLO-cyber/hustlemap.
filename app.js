...
function startJoinedCounter(){
  ...
  let current = 1000; // Initialize to 1000
  
  const updateCounter = () => {
    // Step size: +1 only per update (never more than +1)
    current += 1;
    
    // Soft cap: when it exceeds 1,040, reset softly to a random value in [1,000 ... 1,005]
    if(current > 1040) {
      current = 1000 + Math.floor(Math.random() * 6); // 1000 to 1005
    }
    ...
    // Self-scheduling: random delay between 8-12 seconds (8000-12000ms)
    const nextDelay = Math.floor(Math.random() * 4000) + 8000; // 8000 to 12000ms
    counterTimer = setTimeout(updateCounter, nextDelay);
  };
  
  // Start the self-scheduling timer immediately
  updateCounter();
}
